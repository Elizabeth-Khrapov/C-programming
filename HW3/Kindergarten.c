...
//**************************************************
Garden** readAllGardensFromFile(char* fileName,int* pGardenCount, int chooseWhichFile)
{
	int count, i;
	FILE *fp;
	Garden** gardens;
...
void	writeGardensToFile(Garden** pGardenList, int gardenCount, char* fileName , int chooseWhichFile)
{
	int i;
	FILE *fp;
...
Child*	getChildAskForId(Garden* pGarden)
{
	...
	pChild = findChildById(pGarden->childPtrArr, pGarden->childCount, id);
	if (pChild == NULL)
		return NULL;
...
void    sortChildByID(Garden** pGardenList , int count) 
{
	int bOK = 0;
	Garden* pGarden = NULL;
	do {
		pGarden = getGardenAskForName(pGardenList, count);
		if (pGarden == NULL)
			printf("This Kindergarten doesn't exist in list\n");
		else
			bOK = 1;
	} while (!bOK);
